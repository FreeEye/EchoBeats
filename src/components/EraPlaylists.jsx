...
function matchSongs(playlist, allSongs) {
  ...
  for (const [artistName, titleName] of playlist.songs) {
    ...
    if (poolNames.has(titleLower)) {
      const candidates = poolNames.get(titleLower)
      // Prefer artist match
      for (const s of candidates) {
        if (seen.has(s.newId)) continue
        const songArtists = (s.artists || []).map(a => (a.name || '').toLowerCase()).join(' ')
        if (songArtists.includes(artLower) ||
            artLower.split(/[\\s/]+/).some(p => p && songArtists.includes(p))) {
          seen.add(s.newId)
          matched.push(s)
          break
        }
      }
      if (matched.length > seen.size) continue
    ...
    <div className="white-card" style={{ marginTop: 12 }}>
      ...
      >
        ...
        <span style={{ fontSize: 12, color: '#8c8c8c' }}>
          {playlistsWithMatches.length} 个合集
        </span>
      </div>
      <div
        style={{
