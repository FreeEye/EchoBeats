...
export default function ArtistsSection() {
  ...
  useEffect(() => {
    getArtists().then(setArtists).catch(() => {}).finally(() => setLoading(false))
  }, [])
        ...
        <div className="white-card" style={{ marginTop: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 0 12px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 600, color: '#f0f0f0' }}>热门歌手</span>
              <span style={{ fontSize: 12, color: '#8c8c8c' }}>{artists.length}位</span>
            </div>
            <Link to="/artists" style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: '#8c8c8c', textDecoration: 'none' }}
